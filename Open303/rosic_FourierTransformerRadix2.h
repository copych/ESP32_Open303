...
namespace rosic
{
  ...
  which seems to be rather efficient. It handles the conversion between floats and Complex numbers
  by means of some hacky pointer trickery which relies on the fact that an object of class Complex 
  can be interpreted as two floats stored in subsequent memory locations. This is true for the 
  ...
  */
  ...
  public:
    ...
    an array of floats of length N, the output array will be an array of complex numbers (class 
    ...
    void transformRealSignal(float *inSignal, Complex *outSpectrum);  

    /** Calculates real and imaginary part of the spectrum as interleaved float buffer: 
    ...
    void transformRealSignal(float *signal, float *reAndIm);  
    ...
    void getRealSignalMagnitudesAndPhases(float *signal, float *magnitudes, float *phases);

    /** Calculates the magnitudes only from a signal (useful for analyzer-stuff). */
    void getRealSignalMagnitudes(float *signal, float *magnitudes);

    /** Transforms a complex conjugate symmetric spectrum (i.e. a spectrum of a real signal) into
    the corresponding real signal. */
    void transformSymmetricSpectrum(Complex *inSpectrum, float *outSignal);
    ...
    symmetric). */
    void transformSymmetricSpectrum(float *reAndIm, float *signal);
    ...
    setBlockSize(). */
    void getRealSignalFromMagnitudesAndPhases(float *magnitudes, float *phases, float *signal);
    ...
    static float binIndexToFrequency(int binIndex, int fftSize, float sampleRate) 
  ...
  protected:
    ...
    float normalizationFactor;  /**< The normalization factor (can be 1, 1/N or 1/sqrt(N)). */

    // work-area stuff for Ooura's fft-routines:
    float *w;                   /**< Table of the twiddle-factors. */
