...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    0.0 .. 1.0 where 0 means pure saw and 1 means pure square. */
    void setWaveform(float newWaveform) { oscillator.setBlendFactor(newWaveform); }

    /** Sets the master tuning frequency for note A4 (usually 440 Hz). */
    void setTuning(float newTuning) { tuning = newTuning; }

    /** Sets the filter's nominal cutoff frequency (in Hz). */
    void setCutoff(float newCutoff); 

    /** Sets the resonance amount for the filter. */
    void setResonance(float newResonance) { filter.setResonance(newResonance); }

    /** Sets the modulation depth of the filter's cutoff frequency by the filter-envelope generator 
    (in percent). */
    void setEnvMod(float newEnvMod);
    ...
    void setDecay(float newDecay) { normalDecay = newDecay; }

    /** Sets the accent (in percent).  */
    void setAccent(float newAccent);
    ...
    void setAmpSustain(float newAmpSustain) { ampEnv.setSustainInDecibels(newAmpSustain); }

    /** Sets the drive (in dB) for the tanh-shaper for 303-square waveform - internal parameter, to 
    be scrapped eventually. */
    void setTanhShaperDrive(float newDrive) 
    ...
    void setSquarePhaseShift(float newShift) { waveTable2.set303SquarePhaseShift(newShift); }

    /** Sets the slide-time (in ms). The TB-303 had a slide time of 60 ms. */
    void setSlideTime(float newSlideTime);
    ...
    void setNormalAttack(float newNormalAttack) 
    { 
    ...
    void setAccentAttack(float newAccentAttack) 
    { 
    ...
    void setAccentDecay(float newAccentDecay) { accentDecay = newAccentDecay; }
    ...
    pure square. */
    float getWaveform() const { return oscillator.getBlendFactor(); }
    ...
    void noteOn(int noteNumber, int velocity, float detune);

    void noteOff(int noteNumber,  float detune) { noteOn( noteNumber, 0, detune ); };
  ...
  protected:
    ...
    void setMainEnvDecay(float newDecay);
    ...
    float tuning;           // master tunung for A4 in Hz
    float ampScaler;        // final volume as raw factor
    float oscFreq;          // frequecy of the oscillator (without pitchbend)
    float sampleRate;       // the (non-oversampled) sample rate
    float level;            // master volume level (in dB)
    float levelByVel;       // velocity dependence of the level (in dB)
    float accent;           // scales all "byVel" parameters
    float slideTime;        // the time to slide from one note to another (in ms)
    float cutoff;           // nominal cutoff frequency of the filter
    float envMod;           // strength of the envelope modulation in percent
    float envUpFraction;    // fraction of the envelope that goes upward
    float envOffset;        // offset for the normalized envelope ('bipolarity' parameter)
    float envScaler;        // scale-factor for the normalized envelope (derived from envMod)
    float normalAttack;     // attack time for the filter envelope on non-accented notes
    float accentAttack;     // attack time for the filter envelope on accented notes
    float normalDecay;      // decay time for the filter envelope on non-accented notes
    float accentDecay;      // decay time for the filter envelope on accented notes
    float normalAmpRelease; // amp-env release time for non-accented notes
    float accentAmpRelease; // amp-env release time for accented notes
    float accentGain;       // between 0.0...1.0 - to scale the 3rd amp-envelope on accents
    float pitchWheelFactor; // scale factor for oscillator frequency from pitch-wheel
    float n1, n2;           // normalizers for the RCs that are driven by the MEG
  ...
  INLINE float Open303::getSample()
  {
    ...
    float instFreq = pitchSlewLimiter.getSample(oscFreq);
    ...
    // set up the filter:
    float mainEnvOut = mainEnv.getSample();
    float tmp1       = n1 * rc1.getSample(mainEnvOut);
    float tmp2       = 0.0f;
    if( accentGain > 0.0f )
      tmp2 = mainEnvOut;
    ...
    float instCutoff = cutoff * powf(2.0f, tmp1+tmp2);
    filter.setCutoff(instCutoff);
    
    float ampEnvOut = ampEnv.getSample();
