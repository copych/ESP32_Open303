...
namespace rosic
{
  ...
  numbers as well as the special cases when one of the operands is real (float).
  ...
  floats re, im and nothing else (the algorithms actually run on buffers of floats).
  ...
  */
  ...
  public:
    ...
    Complex& operator*=(const Complex &z)
    {
      float reNew = re*z.re - im*z.im;
      float imNew = re*z.im + im*z.re;
      this->re     = reNew;
      this->im     = imNew;
      return *this;
    }
    ...
    Complex& operator/=(const Complex &z)
    {
      float scale = 1.0 / (z.re*z.re + z.im*z.im);
      float reNew = scale*( re*z.re  + im*z.im  );
      float imNew = scale*( im*z.re  - re*z.im  );
      this->re     = reNew;
      this->im     = imNew;
      return *this;
    }
    ...
    void setRadiusAndAngle(float newRadius, float newAngle);
  ...
  INLINE Complex operator+(const Complex &z, const float &r)
  { return Complex(z.re+r, z.im); }

  /** Adds a real and a complex number. */
  INLINE Complex operator+(const float &r, const Complex &z)
  { return Complex(z.re+r, z.im); }
  ...
  INLINE Complex operator/(const Complex &z, const Complex &w)
  { 
    float scale = 1.0 / (w.re*w.re + w.im*w.im);
  ...
  INLINE Complex operator/(const float &r, const Complex &z)  
  {
    float scale = r / (z.re*z.re + z.im*z.im);
    return Complex(scale*z.re, -scale*z.im);
  }
